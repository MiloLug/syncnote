...
export const Indent = Extension.create<IndentOptions>({
    ...
    addCommands() {
        return {
            indent: () => ({ tr, state, dispatch }): boolean => {
            ...
            outdent: () => ({ tr, state, dispatch }): boolean => {
                const { selection } = state;
                tr = tr.setSelection(selection);
            ...
            insertTabSpaces: () => ({ tr, state, dispatch }: any): boolean => {
                tr = tr.insertText("    ");
                dispatch && dispatch(tr);
                return true;
            }
    ...
    addKeyboardShortcuts(): any {
        return {
            Tab: () => {
                if (this.editor.isActive('code') || this.editor.isActive('codeBlock'))
                    return this.editor.commands.insertTabSpaces();
